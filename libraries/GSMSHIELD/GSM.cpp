...
int GSM::begin(long baud_rate){
    ...
    {
        Serial.print(F("cont loop:"));
        Serial.println(cont);
		if (!turnedON)
        {
            //prob with original code is that even with module off, software serial receives garbage.
	    ...
	    // there is no response => turn on the module
			#ifdef DEBUG_ON
				Serial.print(F("DB:NO RESP power cycle:"));
                Serial.println(cont);
			#endif
                ...
                byte no_of_attempts)
{
  ...
  for (i = 0; i < no_of_attempts; i++) {
    ...
    #ifdef DEBUG_ON
        Serial.println(F("sent, about to wait"));
    #endif
